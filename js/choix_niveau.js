...
n_table.append($('<tr>').append($('<td>').text('id')).append($('<td>').append($('<input>').attr('id','chid').attr('type','text'))));
n_table.append($('<tr>').append($('<td>').text('password')).append($('<td>').append($('<input>').attr('id','chmdp').attr('type','text'))));  
    ...
    if (lfile.last==0)
     ...
     for(var i = 0 , nb=0 ; i < lfile.data.length; i++ )
      ...
      row.click(function(event)
       ...
       console.log('file='+lfile.data[i].name);
       level=lfile.level; tlevel[level]=lfile.data[i].json;
       xhttp.open("GET", lfile.data[i].json, true);
       xhttp.send();
       });
